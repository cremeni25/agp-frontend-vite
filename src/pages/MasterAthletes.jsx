...
export default function MasterAthletes() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const initialParticipants = await supabase.from("agp_participantes_projeto").select("id,pessoa_id,projeto_id,status_onboarding,ativo,tecnico_responsavel_pessoa_id").eq("funcao_no_projeto", "atleta").order("created_at", { ascending: false });
