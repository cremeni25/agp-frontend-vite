...
const EMPTY_FORM = { nome: "", data_nascimento: "", email_contato: "", telefone_contato: "", papel: "atleta", instituicao_id: "", projeto_id: "", esporte_id: "", modalidade: "", prova_posicao: "", categoria: "", nivel: "", status_federativo: "nao_informado", federacao_nome: "", registro_federativo: "", tecnico_responsavel_pessoa_id: "" };
...
const SPORT_LEVELS = [{ value: "SAUDE", label: "Saúde" }, { value: "COMPETITIVO", label: "Competitivo" }, { value: "ALTO_RENDIMENTO", label: "Alto rendimento" }];
...
export default function MasterParticipants() {
  ...
  const [technicianWorkingId, setTechnicianWorkingId] = useState("");
  ...
  async function loadParticipants(projectId) {
    ...
    try {
      ...
      setTechnicianDrafts(Object.fromEntries(rows.filter((item) => item.funcao_no_projeto === "atleta").map((item) => [item.participante_id, item.tecnico_responsavel_pessoa_id || ""])));
      if (requestedParticipant) {
        const athlete = rows.find((item) => item.participante_id === requestedParticipant);
        const current = (baselineRows || []).find((item) => item.participante_id === requestedParticipant);
        if (athlete) setBaselineForm(baselineFromAthlete(athlete, current));
      }
    } catch (requestError) {
      setParticipants([]); setConsents([]); setBaselines([]); setEligibility([]);
  ...
  async function loadBase() {
    ...
    try {
      ...
      setInstitutions(institutionRows); setProjects(projectRows); setSports(sportResult.data || []); setModalities(modalityResult.data || []); setSportHistory(historyResult.data || []); setTechnicalTeam(teamRows || []);
      ...
      if (institutionId || selectedProject) setForm((current) => ({ ...current, instituicao_id: institutionId, projeto_id: selectedProject?.id || "" }));
  ...
  async function submit(event) {
    ...
    if (form.papel === "atleta" && (!form.esporte_id || !form.modalidade)) return setError("Selecione esporte e modalidade do atleta.");
    setWorking(true); setMessage(""); setError("");
    const selectedSport = sports.find((item) => String(item.id) === String(form.esporte_id));
    try {
      const result = await createInstitutionParticipant(form.instituicao_id, {
        ...
        perfil_esportivo: form.papel === "atleta" ? { modalidade: form.modalidade, prova_posicao: form.prova_posicao || null, categoria: form.categoria || null, nivel: form.nivel || null, status_federativo: form.status_federativo, federacao_nome: form.status_federativo === "federado" ? form.federacao_nome || null : null, registro_federativo: form.status_federativo === "federado" ? form.registro_federativo || null : null, dados_complementares: { esporte_id: form.esporte_id, esporte_nome: selectedSport?.nome || null, esporte_slug: selectedSport?.slug || null } } : null
      });
  ...
  async function changeTechnician(participant) {
    const technicianId = technicianDrafts[participant.participante_id] || "";
    if (!technicianId) return setError("Selecione o técnico responsável na ficha do atleta.");
    setTechnicianWorkingId(participant.participante_id); setMessage(""); setError("");
    try {
  ...
  async function grantConsent(participant) {
    ...
    try { await grantParticipantConsent(participant.participante_id, { ...consentForm, escopo: { coleta_prontidao_diaria: true, analise_longitudinal: true, uso_institucional: true } }); setMessage(`Consentimento registrado para ${participant.nome}.`); await loadParticipants(form.projeto_id); }
  ...
  function selectBaselineAthlete(participantId) {
    ...
    setBaselineForm(baselineFromAthlete(athlete, baselineByParticipant[participantId]));
  }
  ...
  async function submitBaseline(event) {
    ...
    try {
      ...
      setMessage(`Linha de base registrada com ${result.completude}% de completude. Estado: ${result.status_onboarding}.`); await loadParticipants(form.projeto_id);
  ...
  return <main className="dashboard-master"><div className="dashboard-overlay master-page">
    ...
    <section className="master-content-grid">
      <form className="master-panel" onSubmit={submit}>
        ...
        {form.papel === "atleta" && <><select className="master-select" required value={form.esporte_id} onChange={(e) => updateField("esporte_id", e.target.value)}><option value="">Selecionar esporte</option>{sports.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select><select className="master-select" required disabled={!form.esporte_id} value={form.modalidade} onChange={(e) => updateField("modalidade", e.target.value)}><option value="">Selecionar modalidade</option>{availableModalities.map((item) => <option key={item.id} value={item.nome}>{item.nome}</option>)}</select><div className="master-toolbar"><input className="master-input" list="agp-provas-posicoes" placeholder="Prova ou posição" value={form.prova_posicao} onChange={(e) => updateField("prova_posicao", e.target.value)} /><input className="master-input" list="agp-categorias" placeholder="Categoria" value={form.categoria} onChange={(e) => updateField("categoria", e.target.value)} /></div><datalist id="agp-provas-posicoes">{positions.map((value) => <option key={value} value={value} />)}</datalist><datalist id="agp-categorias">{categories.map((value) => <option key={value} value={value} />)}</datalist><select className="master-select" required value={form.nivel} onChange={(e) => updateField("nivel", e.target.value)}><option value="">Selecionar nível esportivo</option>{SPORT_LEVELS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><div className="master-toolbar"><select className="master-select" value={form.status_federativo} onChange={(e) => updateField("status_federativo", e.target.value)}><option value="nao_informado">Status federativo não informado</option><option value="vinculado">Atleta vinculado</option><option value="federado">Atleta federado</option></select>{form.status_federativo === "federado" && <><input className="master-input" placeholder="Federação" value={form.federacao_nome} onChange={(e) => updateField("federacao_nome", e.target.value)} /><input className="master-input" placeholder="Registro federativo" value={form.registro_federativo} onChange={(e) => updateField("registro_federativo", e.target.value)} /></>}</div><small>O status federativo contextualiza treino e inscrição em campeonatos, mas não determina sozinho a elegibilidade de uma competição específica.</small><select className="master-select" value={form.tecnico_responsavel_pessoa_id} onChange={(e) => updateField("tecnico_responsavel_pessoa_id", e.target.value)}><option value="">Técnico responsável ainda não definido</option>{technicians.map((item) => <option key={item.pessoa_id} value={item.pessoa_id}>{item.nome} · {item.instituicao?.nome || "Instituição"}</option>)}</select></>}
    ...
    <section className="master-panel"><div className="master-section-heading"><div><span className="master-eyebrow">Projeto selecionado</span><h2>Elegibilidade oficial dos participantes</h2></div><strong>{participants.length}</strong></div>
      {!form.projeto_id ? <div className="master-empty">Selecione um projeto para consultar sua composição.</div> : participants.length === 0 ? <div className="master-empty">Nenhum participante canônico registrado neste projeto.</div> : <ul className="master-activity-list">{participants.map((item) => {
        ...
        return <li key={item.participante_id}><div style={{ width: "100%" }}><strong>{item.nome}</strong><span>{item.funcao_no_projeto}{isAthlete ? ` · ${item.status_federativo === "federado" ? "federado" : item.status_federativo === "vinculado" ? "vinculado" : "status federativo não informado"}` : ""} · coleta {state.apto_coleta ? "liberada" : "bloqueada"} · análise {state.apto_analise ? "liberada" : "bloqueada"}</span>{isAthlete && pending.length > 0 && <small>Pendências: {pending.join(" · ")}</small>}
          {isAthlete && <div className="master-toolbar" style={{ marginTop: "12px" }}><select className="master-select" value={technicianDrafts[item.participante_id] || ""} onChange={(e) => setTechnicianDrafts((current) => ({ ...current, [item.participante_id]: e.target.value }))}><option value="">Selecionar técnico responsável</option>{technicians.map((technician) => <option key={technician.pessoa_id} value={technician.pessoa_id}>{technician.nome} · {technician.papel_canonico || technician.papel}</option>)}</select><button className="master-button" disabled={technicianWorkingId === item.participante_id || !technicianDrafts[item.participante_id]} onClick={() => changeTechnician(item)}>{technicianWorkingId === item.participante_id ? "Salvando..." : item.tecnico_responsavel_pessoa_id ? "Alterar técnico" : "Vincular técnico"}</button><button className="master-button secondary" onClick={() => toggleTechnicianHistory(item.participante_id)}>Histórico técnico</button></div>}
          {Array.isArray(history) && <div className="master-panel" style={{ marginTop: "10px" }}><strong>Histórico técnico do atleta</strong>{history.length === 0 ? <p>Nenhuma troca de técnico registrada.</p> : history.map((entry) => <p key={entry.id}><b>{entry.estado_novo?.tecnico_nome || "Técnico"}</b> · {new Date(entry.created_at).toLocaleString("pt-BR")}<br /><small>{entry.estado_anterior?.tecnico_nome ? `Anterior: ${entry.estado_anterior.tecnico_nome}. ` : ""}{entry.estado_novo?.motivo || "Alteração operacional"}</small></p>)}</div>}
