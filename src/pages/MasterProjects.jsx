...
export default function MasterProjects(){
 ...
 async function load(){setLoading(true);setError("");try{const[i,p]=await Promise.all([listInstitutions(),listProjects()]);setInstitutions(i||[]);setProjects(p||[]);const preferred=contextualHomologacao?(i||[]).find(x=>x.tipo==="homologacao"):(i||[])[0];setForm(current=>({...current,instituicao_id:current.instituicao_id||preferred?.id||"",status:contextualHomologacao?"homologacao":current.status}));}catch(e){setError(e.message)}finally{setLoading(false)}}
 useEffect(()=>{load()},[]);
 ...
 function reset(){setEditingId(null);const preferred=contextualHomologacao?institutions.find(x=>x.tipo==="homologacao"):institutions[0];setForm({...EMPTY,instituicao_id:preferred?.id||"",status:contextualHomologacao?"homologacao":"preparacao"})}
 async function submit(e){e.preventDefault();setSaving(true);setError("");setMessage("");const payload={...form,metodologia:form.metodologia||null,diretrizes:form.diretrizes||null,localidade:form.localidade||null,data_inicio:form.data_inicio||null,data_fim:form.data_fim||null};try{if(editingId){await updateProject(editingId,payload);setMessage("Projeto atualizado.")}else{await createProject(payload);setMessage("Projeto criado.")}reset();await load();if(contextualHomologacao)navigate("/master/homologacao")}catch(x){setError(x.message)}finally{setSaving(false)}}
 ...
 return <main className="dashboard-master"><div className="dashboard-overlay master-page">
  ...
  <section className="dashboard-section master-split"><form className="master-panel" onSubmit={submit}><span className="master-eyebrow">Dados do Master</span><h2>{editingId?"Editar projeto":"Novo projeto"}</h2>
   ...
   <label>Status<select name="status" value={form.status} onChange={change}>{Object.entries(STATUS_LABEL).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
