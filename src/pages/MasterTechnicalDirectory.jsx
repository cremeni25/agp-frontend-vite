...
import { listTechnicalTeam } from "../services/technicalTeamManagement";
...
export default function MasterTechnicalDirectory() {
  ...
  async function load() {
    ...
    try { setMembers(await listTechnicalTeam()); }
  ...
  return <main className="dashboard-master"><div className="dashboard-overlay master-page">
    <header className="dashboard-header master-header"><div><span className="master-eyebrow">Administração</span><h1>Comissões técnicas</h1><p>Consulta global dos profissionais vinculados às instituições do AGP.</p></div><div className="master-header-actions"><button className="master-button secondary" onClick={() => navigate("/dashboard-master")}>Voltar</button><button className="master-button" onClick={load}>Atualizar</button></div></header>
