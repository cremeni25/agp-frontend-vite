...
import { createTechnicalMember, deleteTechnicalMember, listTechnicalTeam, updateTechnicalMember } from "../services/technicalTeamManagement";
  ...
  return <main className="dashboard-master"><div className="dashboard-overlay master-page">
    ...
    <section className="dashboard-section master-split">
      <form className="master-panel" onSubmit={submit}>
        ...
        <div className="master-row-actions"><button className="master-button" disabled={saving}>{saving ? "Salvando..." : editingId ? "Salvar alterações" : "Vincular profissional"}</button>{editingId && <button type="button" className="master-button secondary" onClick={reset}>Cancelar</button>}</div>
      </form>
