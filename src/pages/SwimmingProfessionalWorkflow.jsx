    ...
    {!loading&&<>
      ...
      <div className="workflow-stack">
        ...
        <section className="swim-panel">
          ...
          {cycles.length?<div className="swim-list">{cycles.map(c=><div className="swim-row" key={c.decisao_id}><div><strong>{c.decisao}</strong><span>{c.dominio} · {c.tipo} · {c.evidencias_total||0} evidência(s)</span><small>{c.continuidade?.proxima_etapa && c.continuidade.proxima_etapa!=="ciclo_fechado" ? "Próxima etapa: "+String(c.continuidade.proxima_etapa).replaceAll("_"," ") : "Ciclo fechado ou sem continuidade pendente"}</small></div><div className="workflow-actions"><span className="swim-pill">{String(c.estado_ciclo||c.decisao_estado||"registrado").replaceAll("_"," ")}</span>{c.continuidade?.proxima_etapa && c.continuidade.proxima_etapa!=="ciclo_fechado" && <button type="button" className="swim-secondary" onClick={()=>{setChain({decisionId:c.decisao_id,interventionId:c.continuidade?.intervencao_id||null,responseId:c.continuidade?.resposta_id||null});setSuccess("Ciclo retomado do ponto em que foi interrompido.");window.scrollTo({top:document.body.scrollHeight*0.55,behavior:"smooth"});}}>Retomar ciclo</button>}</div></div>)}</div>:<div className="swim-empty">Nenhum ciclo de decisão registrado.</div>}
        </section>
      </div>
    </>}
  </SwimmingShell>
